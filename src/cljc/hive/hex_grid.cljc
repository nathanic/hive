(ns hive.hex-grid
  ;; (:require [clojure.core.typed :as t :refer [defalias ann U Vec Map Seq Option]])
  (:require [clojure.set :refer [map-invert]]  ; why is this in clojure.set??
            [hive.util :refer [check-argument]]))

; sadly have to remove the core.typed stuff because of figwheel dependency woes
...
#_(ann DIRS (Vec Direction))
(def DIRS [:ne :e :se :sw :w :nw])

#_(ann DIR-VECTORS (Map Direction AxialVector))
...
#_(ann DIR-OPPOSITES (Map Direction Direction))
(def DIR-OPPOSITES
  {:ne :sw
   :e  :w
   :se :nw
...
#_(ann round [Number -> Long])
(defn- round [x]
  (-> x double Math/round))

#_(ann abs [Number -> Double])
(defn- abs [x]
  (-> x double Math/abs))

#_(ann opposite-direction [Direction -> Direction])
(defn opposite-direction
  [dir]
  (or (DIR-OPPOSITES dir) :e))

#_(ann neighbor [AxialPoint Direction -> AxialPoint])
(defn neighbor
  ...
  (if-let [[dp dq] (get DIR-VECTORS dir)]
    [(+ p dp) (+ q dq)]
    (check-argument false (str "Invalid direction: " dir))))
...
#_(ann neighbors [AxialPoint -> (Seq AxialPoint)])
(defn neighbors
...
#_(ann cube-distance [CubePoint CubePoint -> Number])
(defn cube-distance
  [[x1 y1 z1] [x2 y2 z2]]
  (-> (+ (abs (- x1 x2))
         (abs (- y1 y2))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Jommetry
#_(ann hex-height-from-width [Number -> Number])
(defn hex-height-from-width [width]
  (/ (* 2 width) (Math/sqrt 3.0)))

#_(ann hex-size-from-width [Number -> Number])
(defn hex-size-from-width [width]
  (/ (double width) (Math/sqrt 3.0)))

#_(ann hex-width-from-size [Number -> Number])
(defn hex-width-from-size [size]
  (* size (Math/sqrt 3.0)))

#_(ann hex-height-from-size [Number -> Number])
(defn hex-height-from-size [size]
  (* size 2.0))

#_(ann hex-dims-from-size [Number -> '[Number Number]])
...
; r = 2/3 * y / size
#_(ann pixel->axial [Number PixelPoint -> AxialPoint])
(defn pixel->axial
  ...
  [(/ (- (* (Math/sqrt 3.0) x) y)
      size
      3.0)
   ;; had to change this or i get a problem compiling for JS
   ; No method in multimethod 'emit-constant' for dispatch value: class clojure.lang.Ratio
   ; as in cljs there is no Ratio, only Number
   (-> y (* (/ 2.0 3)) (/ size)) ])
...
#_(ann axial->pixel [Number AxialPoint -> PixelPoint])
(defn axial->pixel [size [p q]]
  ; assuming both coordinate systems share the same origin
  [(* size (Math/sqrt 3.0) (+ p (/ q 2.0)))
   (* size (/ 3.0 2) q)])

#_(ann axial->cube [AxialPoint -> CubePoint])
(defn axial->cube [[p q]]
  [p q (- 0 p q)])

#_(ann cube->axial [CubePoint -> AxialPoint])
...
#_(ann cube-round [CubePoint -> CubePoint])
(defn cube-round
...
#_(ann axial-coords-covering-rect [Number Number Number -> (Seq AxialPoint)])
...
#_(ann neighbors? [AxialPoint AxialPoint -> boolean])
...
#_(ann rotate-vector-60 [CubeVector Rotation -> CubeVector])
...
#_(ann plus [CubePoint CubeVector -> CubePoint])
(defn plus
  [[x y z] [dx dy dz]]
  [(+ x dx) (+ y dy) (+ z dz)])
; (mapv - [3 2 1] [4 5 6]) also works but i had trouble getting the types right

#_(ann minus [CubePoint CubePoint -> CubeVector])
(defn minus
  [[x1 y1 z1] [x2 y2 z2]]
  [(- x1 x2) (- y1 y2) (- z1 z2)])

#_(ann gate-positions [AxialPoint AxialPoint -> '[AxialPoint AxialPoint]])
(defn gate-positions
  [apq bpq]
...
#_(ann clojure.set/map-invert (All [a b] [(Map a b) -> (Map b a)]))

#_(ann find-direction-from-axial-points [AxialPoint AxialPoint -> (Option Direction)])
